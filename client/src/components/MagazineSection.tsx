      ...
      <div className="cnp-container">
        <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="mt-4 cnp-title text-foreground">Les recits autour du salon</h2>
          </div>
          <Link href="/magazine">
            ...
            <Link key={article.id} href={`/magazine/${article.id}`} className="group">
              <article className="cnp-card overflow-hidden">
                ...
                <div className="p-5">
                  ...
                  <h3 className="mt-3 text-xl font-bold text-foreground transition group-hover:text-primary">
                    {article.title}
                  </h3>
                  <p className="mt-3 line-clamp-3 text-sm text-foreground/65">{article.excerpt}</p>
                  <p className="mt-4 text-xs uppercase tracking-[0.18em] text-foreground/40">{article.date}</p>
                </div>
              </article>
            </Link>
          ))}
